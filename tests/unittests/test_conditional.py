...
def test_conditional_metrics():
    ...
    assert not np.isclose(cgm.pos_pred_rate(array1,array2,array3,array4),
                           cpdr(array1,array2,array3,array4))
...
def test_conditional_is_consistent():
    ...
    assert np.isclose(pdr[0,0],0.30,atol=0.01)# discrepency in original book vs data
    assert np.isclose(pdr[0,1],0.44,atol=0.01)
    cpdr = cgm.pos_pred_rate.per_group(complete['Prediction'],complete['Accept'],complete['Gender'], complete['Department'])
    assert np.isclose(cpdr[0,0],0.43,atol=0.01)
    assert np.isclose(cpdr[0,1],0.38,atol=0.01)
    

def test_class(use_fast=True):
    fpredictor = fair.FairPredictor(predictor, test_data, 'sex',conditioning_factor='race',use_fast=use_fast)
